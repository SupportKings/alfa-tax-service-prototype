...
class StripeBackfillRunner {
	...
	private printSummary(): void {
		...
		if (failed > 0) {
			console.log("\n❌ Failed entities:");
			for (const r of this.results.filter((r) => r.status === "error")) {
...
function parseArgs(): ScriptOptions {
	...
	for (let i = 0; i < args.length; i++) {
		switch (args[i]) {
			case "--chunk-days":
				options.chunkDays = Number.parseInt(args[++i], 10) || 30;
				break;
			case "--max-retries":
				options.maxRetries = Number.parseInt(args[++i], 10) || 3;
				break;
			case "--delay-ms":
				options.delayMs = Number.parseInt(args[++i], 10) || 1000;
				break;
			case "--start-from":
