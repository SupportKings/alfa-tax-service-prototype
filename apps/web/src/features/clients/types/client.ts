...
export const getFieldValidator = (fieldName: keyof ClientFormInput) => {
	...
	return (value: unknown) =>
		validateSingleField(value, fieldSchemas[fieldName]);
};
...
// Type guard for nested error objects
const hasErrors = (value: unknown): value is { _errors: string[] } => {
	return (
		typeof value === "object" &&
		value !== null &&
		"_errors" in value &&
...
export const getAllValidationErrors = (
	...
	if (validationErrors._errors && Array.isArray(validationErrors._errors)) {
		errors.push(...(validationErrors._errors as string[]));
	}
	...
	Object.entries(validationErrors).forEach(([field, fieldErrors]) => {
		if (field !== "_errors" && fieldErrors) {
			...
			} else if (hasErrors(fieldErrors)) {
				errors.push(
					...fieldErrors._errors.map((error) => `${fieldName}: ${error}`),
				);
			}
		}
	});
