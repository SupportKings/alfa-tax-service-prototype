...
> {
	...
	accessor<TNewVal>(
		...
		const newInstance = this.clone() as unknown as ColumnConfigBuilder<
			...
			TId
		>;
		newInstance.config.accessor = accessor as unknown as TAccessorFn<
			TData,
			TNewVal
		>;
		return newInstance;
	...
	icon(value: LucideIcon): ColumnConfigBuilder<TData, TType, TVal, TId> {
		const newInstance = this.clone();
	...
	> {
		...
		const newInstance = this.clone() as unknown as ColumnConfigBuilder<
			...
			TId
		>;
		newInstance.config.transformOptionFn =
			fn as typeof newInstance.config.transformOptionFn;
		return newInstance;
	}
...
export function createColumns<TData>(
	...
	return columnConfigs.map((columnConfig) => {
		const getOptions: () => ColumnOption[] = memo(
			() =>
				[data, strategy, columnConfig.options] as [
					TData[],
					FilterStrategy,
					ColumnOption[] | undefined,
				],
			([currentData, currentStrategy]) =>
				getColumnOptions(columnConfig, currentData, currentStrategy),
		...
		const getUniqueValues: () => Map<string, number> | undefined = memo(
			() =>
				[getValues() as string[] | ColumnOption[], strategy] as [
					string[] | ColumnOption[],
					FilterStrategy,
				],
			([values, currentStrategy]: [
				string[] | ColumnOption[],
				FilterStrategy,
			]) => getFacetedUniqueValues(columnConfig, values, currentStrategy),
