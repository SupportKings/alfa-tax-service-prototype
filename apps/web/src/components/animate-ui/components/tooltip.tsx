	...
	return (
		<div
			className={cn(
				...
				(side === "top" || side === "bottom") && "-translate-x-1/2 left-1/2",
				(side === "left" || side === "right") && "-translate-y-1/2 top-1/2",
				side === "top" && "-bottom-[3px]",
				side === "bottom" && "-top-[3px]",
				side === "left" && "-right-[3px]",
				side === "right" && "-left-[3px]",
				...
				<TooltipPortal>
					...
					>
						<motion.div
							...
							transition={transition}
							className="relative w-fit text-balance rounded-md bg-primary fill-primary px-3 py-1.5 text-primary-foreground text-sm shadow-md"
						>
...
function TooltipTrigger({ children }: TooltipTriggerProps) {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: intentional - only update tooltip when content/arrow changes, not on every position prop change
	React.useEffect(() => {
