...
const parseData = (
	...
	data.map((dataPoint) => ({
		...dataPoint,
		color:
			categoryColors.get(String(dataPoint[category])) ||
...
const calculateDefaultLabel = (
	...
	sumNumericArray(data.map((dataPoint) => dataPoint[valueKey] as number));
							...
							<Tooltip
								...
								content={({ active, payload }) => {
									const cleanPayload: PayloadItem[] = payload
										? (payload as unknown as PiePayloadItem[]).map((item) => ({
												category: String(item.payload[category]),
												value: item.value,
												color: (categoryColors.get(
													String(item.payload[category]),
												) ??
													AvailableChartColors[0]) as AvailableChartColorsKeys,
											}))
										: [];
