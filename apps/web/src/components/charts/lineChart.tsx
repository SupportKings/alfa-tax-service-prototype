...
const LegendItem = ({
	...
	return (
		<li
			...
			onClick={hasOnValueChange ? handleClick : undefined}
			onKeyDown={hasOnValueChange ? handleKeyDown : undefined}
...
// Extract ChartLegend as a separate component to fix hooks issue
		...
		>
			<Legend
				...
				colors={legendPayload.map(
					(entry) => categoryColors.get(entry.value) || "gray",
				)}
...
const ChartLegend = (
	{ payload }: { payload: LegendPayloadItem[] },
	categoryColors: Map<string, AvailableChartColorsKeys>,
	setLegendHeight: React.Dispatch<React.SetStateAction<number>>,
	activeLegend: string | undefined,
	onClick?: (category: string, color: string) => void,
			...
			>
				...
				<div className={cx("space-y-1 px-4 py-2")}>
					{legendPayload.map(({ value, category, color }, index) => (
						<div
							key={`tooltip-${category}-${value}-${index}`}
...
const LineChart = React.forwardRef<HTMLDivElement, LineChartProps>(
	(props, ref) => {
		...
		function onDotClick(itemData: DotProps, event: React.MouseEvent) {
						...
						<Tooltip
							...
							content={({ active, payload, label: rawLabel }) => {
								...
								const cleanPayload: TooltipProps["payload"] = payload
									? (payload as unknown as TooltipPayloadItem[]).map(
											(item) => ({
												category: item.dataKey,
												value: item.value,
												index: item.payload[index] as string,
							...
							<Line
								...
								dot={
									((props: DotProps) => {
										...
										return (
											<React.Fragment key={`dot-${category}-${dotIndex}`} />
										);
									}) as unknown as undefined
								}
								key={category}
						...
						{onValueChange
							? categories.map((category) => (
									<Line
										...
										connectNulls={connectNulls}
										onClick={
											((props: LineClickProps, event: React.MouseEvent) => {
												event.stopPropagation();
												const { name } = props;
